...
deploy() {
    ...
    if [ "$SERVER_FLAG" = true ]; then
        ...
        ssh "$SERVER_USER@$SERVER_HOST" "rm -rf $SERVER_PATH/html/zensical.tar.gz" > /dev/null 2>&1
        tar -zcvf zensical.tar.gz site > /dev/null 2>&1
        echo -ne "开始传输...                          \r"
        scp zensical.tar.gz "$SERVER_USER@$SERVER_HOST:$SERVER_PATH/html/"
        rm -rf "$CACHE_DIR" "$SITE_DIR" zensical.tar.gz > /dev/null 2>&1
        ssh "$SERVER_USER@$SERVER_HOST" \
            "find '$SERVER_PATH/html' -mindepth 1 ! -name 'zensical.tar.gz' -exec rm -rf {} +" > /dev/null 2>&1
        echo -ne "开始解压...                          \r"
        ssh "$SERVER_USER@$SERVER_HOST" \
            "tar -zxvf '$SERVER_PATH/html/zensical.tar.gz' -C '$SERVER_PATH/html' --strip-components=1" > /dev/null 2>&1

        ssh "$SERVER_USER@$SERVER_HOST" \
            "rm -f '$SERVER_PATH/html/zensical.tar.gz'"

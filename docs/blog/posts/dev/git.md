...
在团队协作或长期维护一个项目时，混乱的 commit message 会让人抓狂——"fix bug"、"改了一下"、"aaa"这类提交在历史记录里几乎毫无价值。Conventional Commits 是目前最主流的提交信息规范，被 Angular、Vue、Vite 等大型开源项目广泛采用，配合工具链还能自动生成 Changelog。

基本格式: 每一条 commit message 遵循以下结构, 其中只有 `type` 和 `subject` 是必填的，其余部分按需填写。

```{linenums="0" .yml .no-copy .no-select}
<type>(<scope>): <subject>
...
subject 是对本次改动的一句话概括，有几个硬性要求：

- 动词开头，使用祈使语气（"添加"而非"添加了"）
- 不超过 50 个字符
- 结尾不加句号
...
scope 写在括号里，用于说明本次改动波及的模块，让读者一眼知道改的是哪里：

```{linenums="0" .yml .no-copy .no-select}
feat(auth): 添加 Google 第三方登录
fix(navbar): 修复移动端菜单无法关闭的问题
style(home): 调整首页导航栏布局
```
...
body 和 subject 之间空一行，用于解释为什么做这次改动，而不是怎么改的——代码本身已经说明了怎么改：

```{linenums="0" .yml .no-copy .no-select}
fix(login): 修复 token 过期后页面不跳转的问题

之前的逻辑只拦截了服务端返回的 401，没有处理
token 在本地已过期的情况，导致用户看似在线，
实际请求已全部失败。
```
...
footer 主要有两个用途。一是关联 Issue，合并后可自动关闭：

```
Closes #42
```

二是标注破坏性变更（Breaking Change），这对维护公共 API 的项目尤为重要：

```
BREAKING CHANGE: 移除了 /api/v1/login 旧版接口，请迁移至 /api/v2/login
```

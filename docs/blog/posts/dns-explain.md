...
于是就独立出一个服务器, 让这台服务器专门去存储这个世界绝大多数的域名和 IP 地址的映射表, 这个服务器就是我们所说的 DNS 服务器。所以在相同网络状况下, 当一个 DNS 服务器的反应时间越快, 网站的资源加载速度就越快, 用户体验就越好。

///html | small
:octicons-light-bulb-16:
在 Windows 中的 **`C:\Windows\System32\drivers\etc\hosts`** 文件中也存有常用的域名与 IP 的对应关系
///

## DNS 的工作原理 {id="How-DNS-works"}

///html | div.step

1. **用户输入域名**：比如在浏览器中输入 **liWanr.com**。

2. **DNS查询**：浏览器会向本地 DNS 服务器发送查询请求, 询问 **liWanr.com** 的 IP 地址是什么。
...
## 常用的 DNS 服务商 {id="commonly-used-DNS-service-providers"}
...
///html | small
:octicons-light-bulb-16:
IPv6 地址是由 8 组 4 位 16 进制的数组成的, 他的隐式值就是把为 0 的组省略, 比如`#!text 2400:3200::1` 的显示值就是 `#!text 2400:3200:0:0:0:0:0:1` , 甚至有些需要 4 位 16 进制, 那就是 `#!text 2400:3200:0000:0000:0000:0000:0000:0001`。因为有些设备要求为 IPv6 地址的所有八个字段提供显式值, 不能接受缩写的 :: IPv6 地址语法。
///

## DNS 污染 / 劫持 {id="DNS-poisoning/hijacking"}
